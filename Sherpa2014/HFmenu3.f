...
C------------J-PROJECTION---------------------------------------
      ...
      real hme
      real,allocatable           :: en(:)
      real,allocatable           :: psdmin(:,:),nsdmin(:,:)
      real emin
      integer imin
...
C...........READ IN HAMILTONIAN...................................
      if(.not.hamflag)then
         ...
         call  make_hvec_iso(nmatpp,nmatnn,nmatpn,
...
333   continue
	...
	if(iout.lt.0 .or.iout.gt. maxseed )then
        ...
        allocate(psdmin(nsps(1),np),nsdmin(nsps(2),nn))

        do i = 1,iout
	  call readinsd(nsps(1),np,tempfile,psdmin,errflag)
	  if(errflag)stop
	  call readinsd(nsps(2),nn,tempfile,nsdmin,errflag)
	  if(errflag)stop
	   do n = 1,nsps(1)
		do m = 1,np
		  psd(n,m)=psdmin(n,m)
...
C      use HAMILTONIAN
      ...
      if(.not.obsflag)then
         ...
         call  make_hvec_iso(nmatpp_O,nmatnn_O,nmatpn_O,
