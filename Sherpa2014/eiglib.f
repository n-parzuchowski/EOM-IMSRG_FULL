...
12      continue
        m=n
2       if(m.ne.l)then
C          if(iter.eq.300)pause 'too many iterations in tqli'
          iter=iter+1
          g=(d(l+1)-d(l))/(2.*e(l))
          r=pythag(g,1.)
...
CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC
      ...
      subroutine eigval(A,n,np,e,vec,work)
C
C  takes in symmetric n x n matrix (phys dimension np x np)
C  computes the eigenvalues in e  and vectors in vec 
C   work is a dummy array 
C   COMPUTES ONLY EIGENVALUES
      implicit none
      ...
      do i =1,n
      	...
      	enddo
      enddo
      call tred2val(vec,n,np,e,work)
      call tqlival(e,work,n,np,vec)
      return
      end

CCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCCC

      SUBROUTINE tred2val(a,n,np,d,e)
      INTEGER n,np
      REAL a(np,np),d(np),e(np)
      INTEGER i,j,k,l
      REAL f,g,h,hh,scale
...
12      continue
        m=n
2       if(m.ne.l)then
C          if(iter.eq.100)pause 'too many iterations in tqli'
          iter=iter+1
          g=(d(l+1)-d(l))/(2.*e(l))
          r=pythag(g,1.)
          g=d(m)-d(l)+e(l)/(g+sign(r,g))
