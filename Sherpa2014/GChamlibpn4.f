...
C OUTPUT
      ...
      logical     ham   ! = true, computes the Hamiltonan
...
C-------- list of packed 4-indices with nonzero TBME     
      integer,pointer,dimension(:)  :: map_combpp_p,map_combnn_p
      integer,pointer,dimension(:)  :: map_combpn_p

C---------- nonzero TBMEs associated with above list   
      real,pointer,dimension(:)     :: hvecpp_p,hvecnn_p,hvecpn_p
      
C---------one-body matrix elements      
      real,pointer,dimension(:,:)  :: e_spepp_p,e_spenn_p
...
C....Make sure the matrices to store Hamiltonian/Observable are not already allocated.....
      ...
      nullify(e_spepp_p,e_spenn_p,hvecpp_p,hvecnn_p,hvecpn_p)
      nullify(map_combpp_p,map_combnn_p,map_combpn_p)
     ...
     &                                       *cleb(1,tc,1,td,2,tkl))
                     ...
                     if(abs(sum).gt.small)then
                        ...
                        if(.not.countonly)then
                           hvecpp_p(nmatpp_p)=sum
                           map_combpp_p(nmatpp_p)=ipack(i,j,k,l)
                        endif
                    end if
                  end if
               end do
...
c         allocate(hvecpp_p(nmatpp_p),map_combpp_p(nmatpp_p))
         if(ham)then
            ...
            hvecpp_p=>hvecpp
            map_combpp_p=>map_combpp
         else
            nmatpp_O=nmatpp_p
            allocate(hvecpp_O(nmatpp_O),map_combpp_O(nmatpp_O))
            hvecpp_p=>hvecpp_O
            map_combpp_p=>map_combpp_O
         endif
         goto 1999
      endif
...
c++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &                          ej(jj,1,ia,ib,ic,id)
                     ...
                     if(abs(sum).gt.small)then
                        nmatnn_p=nmatnn_p+1
                        if(.not.countonly)then
                           hvecnn_p(nmatnn_p)=sum
                           map_combnn_p(nmatnn_p)=ipack(i,j,k,l)
                        endif
                    end if
                  end if
               end do
...
c         allocate(hvecnn_p(nmatnn_p),map_combnn_p(nmatnn_p))
         if(ham)then
            ...
            hvecnn_p=>hvecnn
            map_combnn_p=>map_combnn
         else
            nmatnn_O=nmatnn_p
            allocate(hvecnn_O(nmatnn_O),map_combnn_O(nmatnn_O))
            hvecnn_p=>hvecnn_O
            map_combnn_p=>map_combnn_O
         endif
         goto 1998
      endif
...
c++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++++
     ...
     &                           ej(jj,0,ia,ib,ic,id))
                     ...
                     if(abs(sum).gt.small)then
                        nmatpn_p=nmatpn_p+1
                        if(.not.countonly)then
                           hvecpn_p(nmatpn_p)=sum
                           map_combpn_p(nmatpn_p)=ipack(j,l,i,k)
                        endif
                    end if
                  end if
               end do
...
c         allocate(hvecpn_p(nmatpn_p),map_combpn_p(nmatpn_p))
         if(ham)then
            ...
            hvecpn_p=>hvecpn
            map_combpn_p=>map_combpn
         else
            nmatpn_O=nmatpn_p
            allocate(hvecpn_O(nmatpn_O),map_combpn_O(nmatpn_O))
            hvecpn_p=>hvecpn_O
            map_combpn_p=>map_combpn_O
         endif
         goto 1997
      endif
